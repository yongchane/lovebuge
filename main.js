...
function addHeroAttackEffect(x, y) {
  const hero = getHero();
  if (hero.id === "woo_chi") {
    addEffect("zap", x, y, "#ffd166");
  } else if (hero.id === "swordmaster") {
    addEffect("slash", x, y, "#d8f7ff");
  } else {
    addEffect("slash", x, y, "#8de3ff");
  }
}
...
function renderShop() {
  ...
  if (state.shopTab !== "skill") {
    const cards = {
      tool: [
        ["도구 강화", "현재 도구의 타격 범위와 점수 보너스 강화", "준비"],
        ["이펙트 강화", "타격 스플래시와 흔들림 강화", "준비"],
      ],
      companion: heroes.map((hero) => [hero.name, `${hero.role} - ${hero.desc}`, hero.id === state.heroId ? "출전중" : "출전"]),
      place: venues.map((venue) => [venue.name, venue.meme || "러브버그 출몰 콘셉트", venue.id === state.venueId ? "선택됨" : "선택"]),
    }[state.shopTab] || [];
    ui.skillShop.innerHTML = cards
      .map((card, index) => `
        <button class="shop-card ${card[2] === "선택됨" ? "equipped" : ""}" type="button" data-placeholder="${index}">
          <span>
            <b>${card[0]}</b>
            <small>${card[1]}</small>
          </span>
  ...
  ui.skillShop.innerHTML = purchasableSkills
    .map((skill) => {
      const unlocked = state.unlockedSkills.includes(skill.id);
      const equipped = state.equippedSkillId === skill.id;
      const locked = state.level < skill.minLevel;
      const afford = state.coins >= skill.cost;
      const status = locked ? `Lv.${skill.minLevel}` : equipped ? "장착중" : unlocked ? "장착" : afford ? `${skill.cost}C` : `${skill.cost}C`;
      return `
        <button class="shop-card ${equipped ? "equipped" : ""}" type="button" data-skill="${skill.id}" ${locked ? "disabled" : ""}>
          <span>
            <b>${skill.name}</b>
